...
export default function HomeScreen({ navigation }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      let result = await fetch(
        "https://0186u6yf60.execute-api.eu-west-2.amazonaws.com/v1/check_location?lon=" +
          location["coords"]["longitude"] +
          "&lat=" +
          location["coords"]["latitude"] +
          "&search_range=" +
          search_distance_values[chipCurIsSelected]
      );
  ...
  async function getReminderTypes() {
    const locations = await firebase_db.collection(auth.currentUser.uid).get();
  ...
  async function handle_completion() {
    ...
    const reminders = await firebase_db.collection(auth.currentUser.uid).get();
  ...
  function MapPopup() {
    ...
    if (markersArray.markers.length == 0) {
      return false;
    }
    if (remindersToDisplay == undefined) {
      return false;
    }
  ...
  const search_distance_buttons = ["  Close  ", " Standard ", "     Far     "];

  const search_distance_values = [100, 500, 1000];
